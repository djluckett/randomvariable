...
Because each result of a call to `%convolution%` produces another random variable, the `reduce` function from the `purrr` package can be used to create sums of independent and identically distributed random variables. A sample mean can be constructed using a series of convolutions and a call to `mapRV`. The sample mean of a sample of size 5 is also a random variable, with the `RV` class.

```{r sampleMean5}
exponential_rv = function() {
  RV(function(x) return(0.5 * exp(-0.5 * x)), 
     type = "PDF", 
     lower = 0, 
     upper = Inf)
}
sample5 = list(X1 = exponential_rv(), 
               X2 = exponential_rv(), 
               X3 = exponential_rv())
sum5 = purrr::reduce(sample5, `%convolution%`)
xbar5 = mapRV(sum5, function(x) x/5)
class(xbar5)
```

Using `randomvariable` functions, we can define a custom function to create a sample mean for an independent and identically distributed sample of `n` exponential random variables. 

```{r exponentialSampleMean}
exponential_rv = function(lambda) { 
    RV(function(x) return((1 / lambda) * exp(-(1 / lambda) * x)), 
       type = "PDF", 
       lower = 0, 
       upper = Inf)
  }
exponential_sample_mean = function(n, lambda) {
  exp_sample = vector(mode = "list", length = n)
  for (i in seq_along(exp_sample)) {
    exp_sample[[i]] = exponential_rv(lambda)
  }
  exp_sum = purrr::reduce(exp_sample, `%convolution%`)
  exp_sum
  mapRV(exp_sum, function(x) x/n)
}

xbar5 = exponential_sample_mean(5, 2)
xbar15 = exponential_sample_mean(15, 2)
```

Random variates from the distribution of a sample mean will begin to appear normally distributed as the sample size grows.

```{r sampleMeanPlots, eval = FALSE}
# TODO: this appears to be revealing a bug in mapRV
xbars5 = generateVariates(xbar5, 100)
xbars15 = generateVariates(xbar15, 100)

xbars5_plot = ggplot() + 
  geom_histogram(aes(x = xbars5)) + 
  xlab("X") + 
  ylab("Count") + 
  theme_bw()

xbars15_plot = ggplot() + 
  geom_histogram(aes(x = xbars15)) + 
  xlab("X") + 
  ylab("Count") + 
  theme_bw()
